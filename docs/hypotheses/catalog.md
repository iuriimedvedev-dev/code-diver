...
| Result summary | Learned gating consistently improves H7-only on the measured splits and can approach Gemini quality at much lower call rate, but current features still over-call compared with the oracle. Route-specific thresholds are cheaper and interpretable, but lower quality than logistic/MLP gates. |
...
## H7.5L - Local Cross-Encoder Gated Rerank
