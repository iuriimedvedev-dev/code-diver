...
## H7.3 - Adaptive Gated LLM Rerank

| Field | Value |
| --- | --- |
| ID | `H7.3` |
| Status | active candidate, not default |
| Motivation | H7 has high top-10 candidate recall but still loses head ordering. Always reranking with Gemini Lite improves quality but adds latency and API cost to every query. |
...
| Metrics | CodeSearchNet seed 17 test: H7 Hit@1/nDCG `0.827`/`0.921`, Gemini always `0.907`/`0.958`, MLP gate `0.913`/`0.962` with `26%` rerank calls and `$0.763/1k`, oracle gate `0.920`/`0.965` with `10.7%` calls. Seed 23: logistic/MLP gate Hit@1 `0.947` vs H7 `0.887`, oracle `0.953`. Seed 42: MLP gate Hit@1 `0.900` vs H7 `0.880`, oracle `0.920`. SWE H7.2 aliases were flat versus H6.1 at Hit@1/10 `0.610`/`0.960`. |
...
| Result summary | Learned gating consistently improves H7-only on the measured splits and can approach Gemini quality at much lower call rate, but current features still over-call compared with the oracle. |
| Decision | Keep and improve. Do not make this exact gate default until it is integrated into live search and validated on SWE/e2e answer benchmarks. |
