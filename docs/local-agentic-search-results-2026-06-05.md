# Local Search-Planning Results - 2026-06-05

## Question

Can Code Diver run the pre-answer search-planning layer fully locally, and how
does that compare with the main baseline: H6.1 local locator plus Gemini 3.1
Flash Lite reranking?
...
```text
user question
-> deterministic H6.1 hybrid file search
-> LLM reranks candidate files
-> answer agent reads outlines, symbols, rg/grep hits, and source ranges
-> answer agent explains the code with evidence
```
...
-> optional search-planning tool loop / rerank layer
```
...
| H6.1 + Gemma 4 E2B local search-planning/rerank | 100 | 0.510 | 0.570 | 0.640 | 0.700 | 0.700 | 0.070 | 0.564 | 0.596 | 11,501 | 12,879 | 406 | 312 | 3,045,868 | local | 0 |
| H6.1 + Gemini 3.1 Flash Lite search-planning/rerank | 100 | 0.740 | 0.860 | 0.860 | 0.860 | 0.860 | 0.277 | 0.797 | 0.813 | 10,660 | 14,375 | 454 | 330 | 2,509,105 | $0.70 | 0 |
...
| Gemma E2B local search-planning loop | `.code-diver/reports/local-agent-axis/local-agent-e2b-100.json` | `.code-diver/traces/orchestrator-search/9b55d64f7f81/agent_gemma4_e2b_local_rerank.jsonl` |
| Gemini Lite search-planning loop | `.code-diver/reports/local-agent-axis/gemini-lite-agent-rerank-100.json` | `.code-diver/traces/orchestrator-search/648799f7565e/agent_gemini_lite_rerank_gemini_lite.jsonl` |
...
search-planning+reranker stage for candidate files yet. It loses `0.33 Hit@5`
...
2. Use **Gemini 3.1 Flash Lite** as a gated precision/rerank layer before answer
   generation, not as an unrestricted search-planning loop for every query.
...
| --- | --- |
| Can Gemma E2B locally run the search-planning+rerank loop? | Yes, technically stable. |
| Is Gemma E2B good enough to rerank candidate files? | No, it demotes too many correct files. |
| Is Gemini Lite better at search-planning+rerank? | Yes, but still too recall-destructive as an always-on loop. |
...
| Answer-agent model | Code-reading answer eval: H6.1 returns files, model reads tools/source, answers question. This needs explanation/helpfulness metrics, not only file Hit@K. |

The product-level fork still needs a separate E2E eval:

```text
reranked candidate files
-> A: bounded outline/symbol/rg/grep/read
-> B: ephemeral syntax-aware code index inside those files
-> final answer judged for correctness, grounding, and completeness
```
...
- Hypotheses named `agent_*` now activate the adaptive early-stop policy.
  Before this fix, tool-loop runs could keep searching after a successful rerank.

The local search-planning run is now measuring the model behavior rather than a broken loop.
