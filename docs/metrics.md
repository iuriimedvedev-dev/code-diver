...
| `pi_usage.total_tokens` | Total input, output, and cache tokens reported by Pi model calls. | Lower is better at equal quality. | Primary token budget metric for AI-orchestrated indexing. |
| `pi_usage.input_tokens` | Prompt, tool result, and context tokens sent to the model. | Lower is better. | High values usually mean tool outputs are too large or too many turns are used. |
| `pi_usage.output_tokens` | Tokens generated by the model. | Lower is better at equal quality. | Measures orchestration verbosity and planning overhead. |
| `pi_usage.total_cost` | Provider cost reported by Pi usage records. | Lower is better at equal quality. | Main dollar-cost metric for orchestrator comparisons. |
| `pi_usage.tool_calls` | Number of tool result batches recorded by Pi. | Depends. | Too few means poor inspection; too many means slow and token-heavy indexing. |
...
3. Among similar MRR, prefer lower `pi_usage.total_tokens`, `pi_usage.total_cost`, and `indexing_duration_ms`.
