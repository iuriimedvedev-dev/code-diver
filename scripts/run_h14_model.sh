#!/usr/bin/env bash
# Runs one H14 text-graph answer evaluation against a locally served model, then judges it.
#
# Two things this script deliberately does not do:
#
#   * It does not carry its own port/model table. The port and model are read out of the
#     config the run will use, so the server it starts is by construction the server the
#     config points at. The previous table drifted: it had no qwen35-4b entry at all, and
#     four different models were mapped onto port 8012 across the config tree.
#   * It does not start the shared infrastructure (Qdrant, the embedding server). Those are
#     long-lived and shared between runs, so it checks them and stops with instructions
#     rather than starting a second copy.
set -euo pipefail

MODEL_LABEL="${1:-}"
JUDGE_LABEL="${2:-qwen35-9b}"
DIR="$(cd "$(dirname "$0")/.." && pwd)"
CASES="${CASES:-100}"
DATASET="${DATASET:-datasets/protogen_answer_cases_100.jsonl}"
JUDGE_WORKERS="${JUDGE_WORKERS:-2}"
CONTEXT_FILES="${CONTEXT_FILES:-4}"
CONTEXT_LINES="${CONTEXT_LINES:-160}"

VALID_LABELS="qwen35-4b qwen35-9b gemma4-e4b gemma4-e2b"
if [ -z "$MODEL_LABEL" ]; then
  echo "usage: $(basename "$0") <model-label> [judge-label]"
  echo "  model-label: $VALID_LABELS"
  echo "  judge-label: qwen35-9b (default, cross-family) | gemma4-12b"
  exit 2
fi
...
[ -f "$CFG" ] || { echo "No config for label '$MODEL_LABEL': $CFG"; echo "Valid: $VALID_LABELS"; exit 2; }
[ -f "$JUDGE_CFG" ] || { echo "No judge config for label '$JUDGE_LABEL': $JUDGE_CFG"; exit 2; }
...
OUT="$DIR/.code-diver/reports/protogen-h14-${MODEL_LABEL}-text-graph-${CASES}${BUDGET_SUFFIX}.json"
...
# for why: a hand-built path here is exactly what silently clobbered a 100-case judged
# baseline with a 10-case re-judge (the case-count suffix was dropped).
...
# from a prior run of this model/budget/case-count.
...
serve() {
  ...
  for i in $(seq 1 150); do
    ...
    # and keeps answering it after loading FAILS. gemma-4-12B ("Model type gemma4_unified
    # not supported" on mlx_lm 0.31.3) passed that check and then hung the judge on every
    # request until its 240s timeout. Probing a real completion is the only honest check.
...
serve "$PORT" "$MODEL" "/tmp/h14-${MODEL_LABEL}-server.log"
...
if [ -f "$OUT" ]; then
  ...
  serve "$JUDGE_PORT" "$JUDGE_MODEL" "/tmp/h14-judge-${JUDGE_LABEL}-server.log"
...
# --- summary -----------------------------------------------------------------------------
cd "$DIR" && uv run python scripts/summarize_h14_run.py "$OUT" --judge-report "$JUDGE_OUT" --label "$MODEL_LABEL"
